# MCE-14-CONTROL
How to use Timer Interrupt For motor control; nakub

ไฟล์ ROBOT ประกอบไปด้วย 3 ไฟล์

    1. ROBOT/Sytemrobot/Sytemrobot.ino คือ ไฟล์ที่เป็นระบบรวมการทำงานของหุ่นทั้งหมด

โดยมีลำดับขั้นตอนการรัน

1.1 หลัง Boot หุ่นจะเข้าถึงการรอความพร้อมจากกล้อง เพื่อทำการปรับ HOME

   มีรายละเอียดย่อยคือ
   หุ่นจะทำการรอ READY จากกล้องก่อนช่วงเวลานึง ถ้าหมดแล้วหุ่นจะข้ามไปทำการปรับ HOME เองทันที่ (กรณีเพื่อกันหุ่น Re boot)

1.2 หลังการปรับ HOME หุ่นจะเข้าสู่การรอรับค่าโดยมี ไฟ สีเหลืองแสดง

    2. ROBOT/TESTROBOTWAB/TESTROBOTWAB.ino คือ ไฟล์ที่เป็นการทดสอบการเคลื่อนที่ของหุ่น 
    
โดยใช้มือเราเป็นคนใส่พิกัด X,Y และกดปุ่ม ไป และ กลับ ด้วยตัวเอง

    3. ROBOT/Tuning PID/Tuning PID.ino คือ    เป็นไฟล์เอาไว้สำหรับการปรับค่า PID หรือการทดสอบระบบ

โดยมี 

3.1    การทดสอบการอ่านค่า ENCODER

3.2    การทดสอบการหมุนของมอเตอร์

3.3    การคำนวณโปรไฟล์การเคลื่อนที่

3.4    การคุมความเร็วมอเตอร์    เอาไว้สำหรับ การจูน PID ความเร็ว

3.5    การทำงานทั้งระบบ 
