...
#define POS_VERIFY_COUNT  10
#define POS_VERIFY_TOL    15.0f   // cm


float verify_x[POS_VERIFY_COUNT];
float verify_y[POS_VERIFY_COUNT];
int   verify_idx = 0;
...
// เพิ่มฟังก์ชันนี้ไว้ก่อน loop()
float median_filter(float* arr, int n) {
    float tmp[POS_VERIFY_COUNT];
...
void loop() {
    ...
    switch (state) {
        ...
        // ═════════════════════════════
      case S_CHECK_HOME:  // ← ไม่มี {
    updateBlinkLED(LED_RED, 500);

    if (millis() - t_req_sent >= 2000) {
        t_req_sent = millis();
    ...
    if (xSemaphoreTake(xMutex, 0) == pdTRUE) {
        if (new_robot_pos) {
            ...
            verify_x[verify_idx] = rx;
            verify_y[verify_idx] = ry;
            verify_idx++;

            if (verify_idx < POS_VERIFY_COUNT) {
                send_request_pos_once();
                t_req_sent = millis();
} else {
    verify_idx = 0;
    ...
    if (!has_homed) {
        // รอบแรก → เชื่อค่าไกลที่สุด
        use_x = verify_x[0];
        use_y = verify_y[0];
        float best_err = sqrtf(use_x*use_x + use_y*use_y);
        for (int i = 1; i < POS_VERIFY_COUNT; i++) {
            float e = sqrtf(verify_x[i]*verify_x[i] + verify_y[i]*verify_y[i]);
            if (e > best_err) {
                best_err = e;
                use_x = verify_x[i];
                use_y = verify_y[i];
        ...
        // รอบหลัง → เชื่อ median ตัด spike
        use_x = median_filter(verify_x, POS_VERIFY_COUNT);
        use_y = median_filter(verify_y, POS_VERIFY_COUNT);
    }

    digitalWrite(LED_RED, LOW);
    float err_cm = sqrtf(use_x*use_x + use_y*use_y);

    if (err_cm > CORRECT_THRESHOLD_CM) {
        reset_motion();
        profile_init(-use_x / 100.0f, -use_y / 100.0f, motion_accel);
        portENTER_CRITICAL(&timerMux); running = true; portEXIT_CRITICAL(&timerMux);
        state = S_CORRECT_HOME;
        setLED(false, true, false);
    } else {
        has_homed = true;  // ← home แล้ว รอบหลังใช้ median
        enter_idle();
    }
}
        } else {
    ...
    if ((current_t >= 2.0f * tacce) && (is_position_reached() || (current_t >= 2.0f * tacce + 1.0f))) {
        ...
        // รอหุ่นหยุดนิ่ง 500ms ก่อนขอพิกัด
        t_wait = millis();
        state = S_SETTLE;   // ← state ใหม่
        setLED(true, false, false);
    }
    break;
...
case S_SETTLE:
    stop_all();
    if (millis() - t_wait >= 1000) {   // รอ 500ms
        enter_check_home();
    }
    break;
